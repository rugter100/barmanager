...
class App(customtkinter.CTk):
    ...
    def create_first_user(self):
        ...
        self._log_info('First User Created')
        ...
        if item_data[5] >= 1:
            ...
            if user_data[4] >= item_data[4]:
                ...
                if id_exists == -1:
                    ...
                    self._log_info(msg=f"Order for user id {user_data[1]} added for item id {item_data[1]}")
                    self.update_order_list()
                else:
                    self._log_error('desktop', msg="Something went wrong with adding the order!",
                                   err="Orderid Found in database", popup=True)
            else:
                self._log_error(msg="User is out of credits!", err="out of credits", popup=True)
        else:
            self._log_error(msg="Item is out of stock!", err="stock empty", popup=True)

    # Probably works. Untested unused. Exists if ever needed | Has only info Logging
    def delete_order(self, id=int):
        self.currentdb.delete(table='orders', filters=f"id = {id}")
        self._log_info(f"order with id {id} deleted!")
        self.update_order_list()
    ...
    def update_order_list(self, data=None):
        try:
            ...
            self._log_info('Orderlist and related info updated')
        except Exception as e:
            self._log_error('Something went wrong while running "update_order_list"', err=e)
    ...
    def add_credits(self):
        try:
            ...
            self._log_info(f"Credits were added to user id {user_data[1]} with username {user_name}")
        except Exception as e:
            self._log_error(msg='Something went wrong while running "add_credits"', err=e)
        self.update_order_list()
    ...
    def admin_password(self):
        ...
        if str(password.get_input()) == self.cfg['admin_password']:
            ...
            self._log_info("Successful admin login")
        else:
            self._log_error(msg="Incorrect Wachtwoord", err="Input does not match saved password", popup=True)
    ...
    def open_admin_menu(self):
        try:
            ...
            self._log_info("Admin panel opened successfully")
        except Exception as e:
            self._log_error(msg="Something went wrong with opening the admin panel", err=e, popup=True)


        self.update_user_list()
        self.update_item_list()

    # Fully Functional | Does not need logging itself
        ...
        try:
            ...
            if name != "":
                ...
                if id_exists == -1:
                    ...
                    self._log_info(f"User {name} with id {id} added")
                else:
                    self._log_error(msg="Something went wrong with adding the user!", err="Userid Found in database", popup=True)
        except Exception as e:
            self._log_error(msg='Something went wrong while running "add_user"', err=e)
    ...
    def delete_user(self, id:int):
        try:
            if self.cfg['debug']:
                ...
                self._log_warn(msg="Debug mode is enabled! User was fully deleted from database instead of being marked as deleted")
            else:
                self.currentdb.update(table='members', filters=f"id = {id}", collumn='deleted', value=1)
            self._log_info(f"User with id {id} was deleted!")
        except Exception as e:
            self._log_error(msg='Something went wrong while running "delete_user"', err=e)
        self.update_user_list()
    ...
    def update_users(self):
        try:
            ...
            for item in self.scrollable_frame_members_data_2:
                ...
                row += 1
            self._log_info(f"Users were updated succesfully")
        except Exception as e:
            self._log_error(msg='Something went wrong while running "update_user"', err=e)
    ...
    def update_user_list(self, skip_admin=False):
        try:
            ...
            self._log_info("Userlist was updated successfully")
        except Exception as e:
            self._log_error(msg='Something went wrong while running "delete_user_list"', err=e)
            ...
            if name != "":
                ...
                if id_exists == -1:
                    try:
                        ...
                        self._log_info(f"Item with id {id} and name {name} was added successfully.")
                        self.update_item_list(skip_admin=skip_admin)
                    except Exception as e:
                        self._log_error(msg="Something went wrong with adding the item!", err=str(e), popup=True)
                else:
                    self._log_error(msg="Something went wrong with adding the item!", err="Itemid Found in database", popup=True)
            else:
                self._log_error(msg="Something went wrong with adding the item!", err="Name is empty or price is not an integer", popup=True)
        except Exception as e:
            self._log_error(msg='Something went wrong while running "add_item"', err=e)
    ...
    def delete_item(self, id:int):
        try:
            if self.cfg['debug']:
                ...
                self._log_warn(msg="Debug mode is enabled! Item was fully deleted from database instead of being marked as deleted")
            else:
                self.currentdb.update(table='items', filters=f"id = {id}", collumn='deleted', value=1)
            self._log_info(f"Item with id {id} was deleted!")
        except Exception as e:
            self._log_error(msg='Something went wrong while running "delete_item"', err=e)
        self.update_item_list()
    ...
    def update_items(self):
        try:
            ...
            for item in self.scrollable_frame_items_data_2:
                ...
                row += 1
            self._log_info(f"Items were updated succesfully")
        except Exception as e:
            self._log_error(msg='Something went wrong while running "update_items"', err=e)

    # Has a bug with deleting buttons. Not sure how to fix yet | Bug should be fixed (not tested) | Has all Logging
        ...
        try:
            ...
            while not cont:
                if f"{column}-{row}" in self.order_buttons:
                    ...
                    cont = True
            self._log_info("Item list was updated successfully")
        except Exception as e:
            self._log_error(msg='Something went wrong while running "update_item_list"', err=e)
    ...
    def _log_error(self, msg:str, err:str='No Error', popup=False):
        if self.logging:
            self.log.info(name='desktop', msg=msg, cmdout=self.info_cmdout)
        if popup:
            self.error_message(name='desktop', msg=msg, err=err)
...
if __name__ == "__main__":
    ...
    with open(r'configs/config.yml') as config:
        ...
        bool_values = {'debug': cfg['debug'],
                       'database/local/initialize': cfg['database']['local']['initialize'],
                       'user_preferences/fullscreen_borderless': cfg['user_preferences']['fullscreen_borderless'],
                       'user_preferences/use_credits': cfg['user_preferences']['use_credits'],
                       'user_preferences/logging': cfg['user_preferences']['logging'],
                       'user_preferences/info_cmdout': cfg['user_preferences']['info_cmdout'],
                       'user_preferences/show_totals': cfg['user_preferences']['show_totals']}

        str_values = {'group_name': cfg['group_name'],
                      'admin_password': cfg['admin_password'],
                      'database/local/db_id': cfg['database']['local']['db_id']}
        for item in cfg['lang']:
            str_values[f"lang/{item}"] = cfg['lang'][item]

        int_values = {'user_preferences/credit_value_in_cents': cfg['user_preferences']['credit_value_in_cents'],
                      'user_preferences/credits_added_via_button': cfg['user_preferences']['credits_added_via_button'],
                      'user_preferences/order_columns': cfg['user_preferences']['order_columns'],
                      'user_preferences/default_ui_scale': cfg['user_preferences']['default_ui_scale']}
